...
async function updateLeadCells(rowNum, updates) {
  ...
  try {
    ...
    console.log('✅ Instant update row ' + rowNum + ': ' + updates.map(u => String.fromCharCode(65 + u.col) + '=' + u.value).join(' | '));
  } catch (e) {
    console.error('❌ Instant update failed:', e.message);
    // Fallback to queue if instant fails
    const regNo = await getRegNoFromRow(rowNum);
    if (regNo) await queueSheetUpdate(regNo, updates);
  }
}
...
async function processSheetBatch() {
  try {
    ...
    for (const p of pending) {
      ...
      if (rowNum && Array.isArray(p.updates)) {
...
function isFinalResponse(text) {
  ...
  if (/\b(?:gadi|car|gaadi)\b/.test(t) && /\b(?:nahi|nhi|na|available)\b/.test(t) && /\b(?:hai|h|rahi|available)\b/.test(t)) return true;
...
async function sendNext(chatId, sName) {
  ...
  for (const lead of pends) {
    ...
    // INSTANT: Update sheet immediately
    try {
      ...
      console.error('Instant sheet update error:', e.message);
    }
